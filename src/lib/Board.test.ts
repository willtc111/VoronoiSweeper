...
describe("Board", () => {
	describe("createBoard", () => {
		...
		it("should throw an error if mineCount >= cellCount", () => {
			const rng = mulberry32(42);
			expect(() => createBoard(10, 10, 5, 5, rng)).toThrow(
				"Number of mines must be less than number of cells"
			);
		});
		...
		it("should calculate neighbor mine counts correctly", () => {
			...
			for (const cell of board.cells) {
				const actualNeighborMines = cell.neighbors.filter(
					(neighborIndex) => board.cells[neighborIndex].isMine
				).length;
